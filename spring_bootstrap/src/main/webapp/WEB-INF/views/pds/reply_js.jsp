...
window.onload=function(){
	getPage("<%=request.getContextPath()%>/reply/list.do?pno=${pds.pno}&page="+replyPage);
	
	$('ul.pagination').on('click','li a',function(event){		
		//alert($(this).attr("href"));
		replyPage=$(this).attr("href");
		
		getPage("<%=request.getContextPath()%>/reply/list.do?pno=${pds.pno}&page="
				+replyPage);
...
function replyRegist_go(){
	...
	$.ajax({
		...
		success:function(data){
			...
			getPage("<%=request.getContextPath()%>/reply/list.do?pno="+pno+"&page="+data); //리스트 출력
			$('#newReplyText').val("");	
		},
		error:function(){
...
function replyRemove_go(){
	...
	$.ajax({
		url:"<%=request.getContextPath()%>/reply/remove.do?rno="+rno
				+"&page="+replyPage+"&pno=${pds.pno}",
		type:"get",
		success:function(page){
			alert("삭제되었습니다.");
			getPage("<%=request.getContextPath()%>/reply/list.do?pno=${pds.pno}&page="+page);
			replyPage=page;
		},
